...
<div style=" max-width:450px; margin:0 auto;margin-top: 70px;">
    ...
    <div style="">
        <ul>
            @foreach(\App\Models\Checkin::where('user_id', auth()->id())->orderByDesc('id')->get() as $element)
    ...
    border-radius: 7px;">
                    <div>
                        <h3 style="font-weight: bold">Promo Bonus</h3>
                        <p>{{$element->created_at}}</p>
                    </div>
                    <div>
                        <h3 style="font-weight: bold">{{price($element->amount)}}</h3>
                        <p class="sposp" style="text-transform: capitalize;">Daily once time</p>
                    </div>
                </li>
            @endforeach
        </ul>
